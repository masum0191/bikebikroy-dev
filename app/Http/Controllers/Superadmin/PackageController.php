...
class PackageController extends Controller
{
            ...
            if (request()->ajax()) {
              return datatables()->of(Package::latest()->get())
                ->addColumn('action', function ($data) {
                  $button = '<button title="Edit Package" id="editBtn" style="border:0; background: none; padding: 0 !important"   rid="' . $data->id . '" class="btn-md"><i class="far fa-edit"></i></button>';
                  $button .= '&nbsp;&nbsp;';
  ...
  public function store(Request $request)
  {
    $validator = Validator::make($request->all(), [
      'packagename' => 'required|min:1|max:190|unique:packages',
...
return response()->json($info);
  ...
  {
    $validator = Validator::make($request->all(), [
      'packagename' => 'required|min:1|max:190|unique:packages,packagename,'.$id,
      'packageprice' => 'required',
      'expiryday' => 'required'
         ]);
         if ($validator->fails()) {
